 ...
 * DESCRIBE:
 *  dragBarClass: 拖动的元素需要绑定的className
 * */
Vue.directive('dragx', (el, binding, vnode) => {
  ...
  let isMove = false
  binding.val = binding.value || {}
  const cfg = Object.assign({}, defaultOpts, binding.val)
  ...
  const getDirection = (e) => {
    ...
    if (binding.value) {
      const directions = cfg.dragDirection.split(',')
      for (let i = 0; i < directions.length; i++) {
        const handle = directions[i].replace(/(^\s*)|(\s*$)/g, '')
  ...
  el.onmousedown = (e) => {
    ...
    document.onmousemove = (edom) => {
      ...
      if (isMove && cfg.canDrag) {
        const targetPageX = edom.pageX
        const targetPageY = edom.pageY
        const deltX = targetPageX - data.startX - data.deltX
        const deltY = targetPageY - data.startY - data.deltY
        const newLeft = parseInt(getStyleNumValue('left') || '0', 10) + deltX
        const newTop = parseInt(getStyleNumValue('top') || '0', 10) + deltY
        data.left = newLeft
        data.top = newTop
